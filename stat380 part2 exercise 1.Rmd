...
kable_styling(kable(results, format = "latex", digits = 3), full_width = TRUE)
```
...
library(corrplot)
corrplot(cor(market_scaled), method = "shade")
```
...
# center 2 seems to be a good choice; 
```
...
library(cluster)
library(fpc)
clusplot(market_scaled, cluster$cluster, color=TRUE, shade=TRUE, labels=2, lines=0, main='cluster plot')
```

In the end, I validated my findings with the following plots. There are clear two clusters in these plots, which proves my findings in the beginning. One group of audience are associated with labels such as photo_sharing, fashion, beauty, cooking, shopping, and chatter; Another group of audience are associated with labels such like school, family, food, parenting, sports_fandom, and dating.

```{r, echo=FALSE}
a1 = ggplot(market, aes(fashion, school, color = cluster$cluster)) + geom_point()

a2 = ggplot(market, aes(beauty, parenting, color = cluster$cluster)) + geom_point()

a3 = ggplot(market, aes(cooking, family, color = cluster$cluster)) + geom_point()

a4 = ggplot(market, aes(school, photo_sharing, color = cluster$cluster)) + geom_point()

a5 = ggplot(market, aes(food, shopping, color = cluster$cluster)) + geom_point()

a6 = ggplot(market, aes(sports_fandom, shopping, color = cluster$cluster)) + geom_point()
